...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        return [self.startingPosition,unvisited_corners,4]
    ...
    def getResult(self, state, action):
        ...
        if(not hitsWall):
            ...
            # copy the state, casue you do not want modify previous visited states
            # ex. ([3,1], [[1,1],[1,6]],2)
            # ret_state = state.copy() will not work because [[1,1],[1,6]] is still passed in as a reference
            # DO NOT mutate states, each node should have its own state
            unvisited_corners=state[1].copy()
            # print(state)
            counter = state[2]
            # nextPosition is corner
            if(nextPosition in state[1]):
                # delete this corner
                unvisited_corners.remove(nextPosition)
                # decrement the counter
                counter=counter-1;
                return [nextPosition, unvisited_corners,counter]
            else:
                return [nextPosition, unvisited_corners,counter]
        else:
            warnings.warn("Warning: checking the result of an invalid state, action pair.")
    ...
    while (len(visited)!=len(unvisited)):
        ...
        for corner in unvisited:
            if(corner not in visited):
                dis = abs(cur_state[0]-corner[0])+abs(cur_state[1]-corner[1])
                dis_arr.append(dis)
            else:
                # keep the dis_arr matching with the unvisited array
                dis_arr.append(-1)
        # print("dis",dis_arr)      
        if(dis_arr !=[]):
            min_heur=min([d for d in dis_arr if d!=-1])
            h+=min_heur
            cur_state=unvisited[dis_arr.index(min_heur)]
...
class FoodSearchProblem:
    ...
    def getStartState(self):
        #self.start[1] is T/F matrix, .count return nums of trues in the matrix
        # print(self.start[0],self.start[1].count())
        #.asList() return the location of foods
...
def foodHeuristic(state, problem):
    ...
    for food in unvisited:
            # if(food not in visited):
                #dis = abs(cur_state[0]-food[0])+abs(cur_state[1]-food[1])
        dis=mazeDistance(cur_state,food,problem.startingGameState)
        h_values=max(h_values,dis)
        #         dis_arr.append(dis)
        #     else:
        #         # keep the dis_arr matching with the unvisited array
        #         dis_arr.append(-1)
        ...
        # if(dis_arr !=[]):
        #     min_heur=min([d for d in dis_arr if d!=-1])
        #     h_values.append(min_heur)
        #     cur_state=unvisited[dis_arr.index(min_heur)]
        #     visited.append(cur_state)
    # print(h)
    # print("---------------------------")
    return h_values # Default to trivial solution
