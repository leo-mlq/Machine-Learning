...
aug_datagen = ImageDataGenerator(preprocessing_function = random_erase, data_format='channels_last',horizontal_flip=True, rotation_range = 5)
...
model_checkpoint = ModelCheckpoint(filepath='./new_models2/mobileNet_checkpoints.h5', verbose=1, save_best_only=True, monitor='val_acc')
callbacks = [model_checkpoint]

history_fit = mobileNet_model.fit_generator(train_data, steps_per_epoch=nb_train_samples/128.0, epochs=400,
                    validation_data=val_data, validation_steps=nb_validation_samples/128.0, callbacks=callbacks)


model_json=mobileNet_model.to_json()
with open("./new_models2/mobileNet_json.json", "w") as json_file:
    json_file.write(model_json)
mobileNet_model.save_weights("./new_models2/mobileNet_weights.h5")
mobileNet_model.save("./new_models2/mobileNet_model.h5")
